...
namespace DiscussionExperimental
{
    ...
    class DiscussionExperimental
    {
        public static string storeName = "experimental-death-penalty-study19";
        ...
        static void Main(string[] args)
        {
            ...
            using (var p = Pipeline.Create(enableDiagnostics: true))
            {
                ...
                var llmResponseInput = templlmResponseInput1.Select(m => m.Data).Merge(isSubmittedLLMInput);
                
                var llmResponseComponent = new LLMResponseComponent(p);
                llmResponseInput.Where(m => isRobotTalking == false && isLLMProcessing == false && isStarted == true && isDone == false).Where(m => m.Data.Item1.Length > 0).Select(m => (m.Data.Item1, m.Data.Item2, m.Data.Item3, m.Data.Item4, m.Data.Item5, currRobotBehaviorString, isEvent, eventNum)).Do(m =>
                {
                    isLLMProcessing = true;
                    isEvent = false;
